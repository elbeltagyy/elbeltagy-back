...
const verifyCoupon = expressAsyncHandler(async (req, res, next) => {
    ...
    const foundCoupon = await CouponModel.findOne({ coupon, isActive: true }).lean().select("discount course usedBy")
    ...
    const foundCourse = await CourseModel.findById(foundCoupon.course).lean().select("price")
